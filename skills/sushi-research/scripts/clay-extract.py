...
def get_saved_session() -> str | None:
    """Load saved cookie if still fresh (< 20 hours)."""
...
def get_clay_cookie_from_env() -> str | None:
    """Read CLAY_COOKIE from .env.clay if it exists."""
    env_file = Path(".env.clay")
...
def get_cookie() -> str:
    ...
    if cookie and test_session(cookie):
        print("[OK] Using Clay session from .env.clay")
        save_session(cookie)
        return cookie
...
class ClayAPI:
    ...
    def search_workbooks(self, workspace_id: str, query: str) -> list[dict]:
        ...
        q = query.lower()
        return [wb for wb in workbooks if q in wb.get("name", "").lower()]
...
def print_summary(extract: dict):
    ...
    for f in fields:
        ...
        ftype = f["type"]
        marker = f" [{action_key}]" if ftype == "action" else (" [source]" if ftype == "source" else "")
...
def main():
    ...
    if info["type"] == "table":
        ...
        if not ws_id:
            ...
            print("  Use: python3 scripts/clay-extract.py --workspace <workspace_id> \"Table Name\"")
        ...
        if not matches:
            print(f"  No workbooks matching \"{query}\" — searching table names...")
        ...
        if info["type"] == "folder":
            print(f"  [NOTE] Folder children aren't exposed via the API.")
